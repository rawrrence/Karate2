...
class RegistrationTest < ActiveSupport::TestCase
	...
	context "Creating seven different registration" do
		setup do
			...
			@sec1 = FactoryGirl.create(:section, :event => @breaking, :min_age => 10, :max_age => 30, :min_rank => 1, :max_rank => 10)
			...
			@reg_ken1 = FactoryGirl.create(:registration, :section => @sec2, :student => @kenneth, :date => Date.today.weeks_ago(4), :fee_paid => true, :final_standing => 3)
			@reg_ken2 = FactoryGirl.create(:registration, :section => @sec3, :student => @kenneth, :date => Date.today.weeks_ago(2), :fee_paid => true, :final_standing => 5)
			@reg_kim1 = FactoryGirl.create(:registration, :section => @sec3, :student => @kenneth, :date => Date.today.weeks_ago(1), :fee_paid => false, :final_standing => 9)
		...
		should "shows the registrations of a particular section" do
			assert_equal 2, Registration.for_section(@sec2.id)
			students_sec2 = Registration.for_section(@sec2.id).map{|r| r.student}
			assert_equal ["Kim", "Peter"], students_sec2.alphabetical.map{|s| s.first_name}
		end
